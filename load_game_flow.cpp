...
namespace load_game_flow_detail
{
    ...
    bool collect_save_slots(
        const ft_string &commander_name, ft_vector<save_slot_entry> &out_slots, ft_vector<ft_string> *out_errors);
        ...
        int &out_level, bool &out_difficulty_valid, int &out_difficulty) noexcept
    {
        ...
        if (campaign_group != ft_nullptr)
        {
            ...
            if (difficulty_item != ft_nullptr)
            {
                ...
                {
                    if (parsed_difficulty == GAME_DIFFICULTY_EASY || parsed_difficulty == GAME_DIFFICULTY_STANDARD
    ...
    ft_string format_difficulty_component(bool difficulty_valid, int difficulty)
    {
        ...
        return menu_localize_format("load_menu.metadata.difficulty_known", "Difficulty: {{difficulty}}", replacements);
    }
        ...
        const ft_string &commander_name, const ft_string &old_name, const ft_string &new_name, ft_string &out_error) noexcept
    {
        ...
        {
            out_error = menu_localize("load_menu.error.name_length", "Save names are limited to 24 characters.");
            return false;
        }
        ...
        {
            out_error = menu_localize("load_menu.error.resolve_existing", "Unable to resolve existing save location.");
            return false;
        }
        ...
        {
            if (source_exists < 0)
                out_error = menu_localize("load_menu.error.verify_existing", "Unable to verify existing save file.");
            else
        ...
        {
            out_error = menu_localize("load_menu.error.resolve_new", "Unable to resolve new save location.");
            return false;
        }
        ...
        {
            out_error = menu_localize("load_menu.error.check_conflict", "Unable to check for conflicting saves.");
            return false;
        }
        if (destination_exists > 0)
        {
        ...
        const ft_string &commander_name, ft_vector<save_slot_entry> &out_slots, ft_vector<ft_string> *out_errors)
    {
        ...
        while ((entry = file_readdir(directory_stream)) != ft_nullptr)
        {
            ...
            if (metadata_loaded)
            {
                ...
                {
                    slot.metadata_label = menu_localize("load_menu.metadata.unavailable", "Metadata unavailable");
            ...
            else
            {
                ...
                if (out_errors != ft_nullptr)
                {
                    ...
                    replacements.push_back(replacement);
                    out_errors->push_back(menu_localize_format("load_menu.errors.metadata_parse",
                        "Save \"{{slot}}\" could not be read (metadata parsing failed).", replacements));
                }
            }
    ...
    void rebuild_load_menu(ft_ui_menu &menu, const ft_vector<save_slot_entry> &slots, bool has_selection)
    {
        ...
        ft_menu_item cancel_item(ft_string("action:cancel"), menu_localize("load_menu.menu.cancel", "Cancel"), cancel_rect);
                ...
                if (text_texture != ft_nullptr)
                {
                    ...
                    if (!item.description.empty())
                    {
                        ...
                        SDL_Texture *description_texture
                            = create_text_texture(renderer, *menu_font, item.description, description_color, description_rect);
...
using load_game_flow_detail::append_save_character;
using load_game_flow_detail::collect_save_slots;
using load_game_flow_detail::delete_save_slot;
using load_game_flow_detail::kMaxSaveNameLength;
using load_game_flow_detail::remove_last_save_character;
using load_game_flow_detail::rename_save_slot;
using load_game_flow_detail::save_name_is_valid;
using load_game_flow_detail::save_slot_entry;
    ...
    ft_string &out_metadata_label, bool &out_metadata_available) noexcept
{
    ...
    ft_vector<save_slot_entry> slots;
    if (!collect_save_slots(commander_name, slots, ft_nullptr))
        return false;
    ...
    const ft_string &commander_name, ft_string &out_selected_save, bool &out_quit_requested)
{
    ...
    bool collected = collect_save_slots(commander_name, slots, ft_nullptr);
    if (!collected)
    {
        status_message = menu_localize("load_menu.status.read_directory_failed",
    ...
    ft_ui_menu menu;
    load_game_flow_detail::rebuild_load_menu(menu, slots, !slots.empty());
    ...
    auto begin_delete_confirmation = [&](const save_slot_entry &slot) {
        ...
        status_message = menu_localize_format(
            "load_menu.status.delete_confirm", "Press Delete again to remove \"{{slot}}\". Esc cancels.", replacements);
        status_is_error = true;
    };
    ...
    while (running)
    {
                ...
                if (rename_active)
                {
                    ...
                    if (text != ft_nullptr)
                    {
                        ...
                        for (int index = 0; text[index] != '\0'; ++index)
                        {
                            ...
                            else if (!load_game_flow_detail::is_save_character_allowed(character))
        ...
        if (rename_requested)
        {
            ...
            {
                status_message
                    = menu_localize("load_menu.status.rename_select", "Select a save before pressing R to rename.");
                status_is_error = true;
            }
        }
        ...
        if (rename_confirm_now && rename_active)
        {
            ...
            const bool renamed
                = rename_save_slot(commander_name, rename_original_label, rename_input, rename_error);
            if (renamed)
            {
                ft_string new_identifier("save:");
                new_identifier.append(rename_input);
                ft_vector<StringTableReplacement> replacements;
                replacements.reserve(2U);
                StringTableReplacement from_replacement;
                from_replacement.key = ft_string("from");
                from_replacement.value = rename_original_label;
                replacements.push_back(from_replacement);
                StringTableReplacement to_replacement;
                to_replacement.key = ft_string("to");
                to_replacement.value = rename_input;
                replacements.push_back(to_replacement);
                status_message = menu_localize_format(
                    "load_menu.status.rename_success", "Renamed save \"{{from}}\" to \"{{to}}\".", replacements);
                status_is_error = false;

                clear_rename_state();

                bool refreshed = collect_save_slots(commander_name, slots, ft_nullptr);
                if (!refreshed)
                {
                    status_message = menu_localize("load_menu.status.refresh_failed_rename",
                        "Unable to refresh save directory after renaming.");
                    status_is_error = true;
                    slots.clear();
                }
            ...
            else
            {
                if (rename_error.empty())
                    status_message = menu_localize("load_menu.error.rename_failed", "Failed to rename save file.");
                else
        ...
        if (keyboard_state.pressed_delete)
        {
            ...
            else
            {
                ...
                {
                    status_message
                        = menu_localize("load_menu.status.delete_select", "Select a save before pressing Delete.");
                    status_is_error = true;
                }
                clear_delete_confirmation();
            }
        ...
        if (confirm_delete_now)
        {
            ...
            if (deleted)
            {
                ...
                replacements.push_back(replacement);
                status_message
                    = menu_localize_format("load_menu.status.delete_success", "Deleted save \"{{slot}}\".", replacements);
                status_is_error = false;
            }
            else
            {
                if (delete_error.empty())
                    status_message = menu_localize("load_menu.error.delete_failed", "Failed to delete save file.");
                else
            ...
            if (!refreshed)
            {
                status_message = menu_localize("load_menu.status.refresh_failed_delete",
            ...
            {
                status_message
                    = menu_localize("load_menu.status.no_saves", "No campaign saves found for this commander.");
                status_is_error = false;
            }
        ...
        if (!skip_menu_activation)
        {
            ...
            {
                const int hovered_index = menu.get_hovered_index();
                const ft_menu_item *hovered_item = load_game_flow_detail::menu_item_from_index(menu, hovered_index);
        ...
        load_game_flow_detail::render_load_screen(
            *renderer, menu, title_font, menu_font, commander_name, status_message, status_is_error);
...
namespace load_game_flow_testing
{
    ...
    {
        return load_game_flow_detail::build_save_file_path(commander_name, slot_name);
    }
    ...
    {
        return load_game_flow_detail::delete_save_slot(commander_name, slot_name, out_error);
    }

    bool rename_save_slot(
        const ft_string &commander_name, const ft_string &old_name, const ft_string &new_name, ft_string &out_error)
    {
        return load_game_flow_detail::rename_save_slot(commander_name, old_name, new_name, out_error);
    }
}
